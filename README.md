...
```bash
bash start.sh          # 普通启动
bash start.sh build    # 重新编译后端再启动
```

脚本自动完成：MySQL/Redis 启动 → 建库建表 → 编译后端 → 启动后端 → 安装前端依赖 → 启动前端。

## SQL 对象（存储过程 / 函数 / 触发器 / 视图）

核心业务逻辑下沉到数据库层，Java 只负责参数校验 + 调用存储过程。

| 类型 | 数量 | 说明 |
|------|:--:|------|
| 存储过程 | 13 | 注册、下单、接单、备餐、取消、收货、配送全流程、审核、删除 |
| 函数 | 3 | 订单号生成、金额计算、状态转换校验 |
| 触发器 | 2 | 库存归零自动下架、订单完成→销量+1+生成配送 |
| 视图 | 7 | 订单详情、商家统计、骑手绩效、菜品销量、每日统计、顾客消费、用户汇总 |

完整 DDL 文档：[`docs/sql/sql-objects.sql`](docs/sql/sql-objects.sql)

```bash
# 一键部署所有 SQL 对象
mysql -u root -p takeout < docs/sql/sql-objects.sql
```
...
| 文档 | 路径 |
|------|------|
| 系统需求分析 | `外卖管理系统-系统需求分析.md` |
| 概念逻辑结构设计 | `外卖管理系统-概念逻辑结构设计-完整版.md` |
| 项目架构文档 | `外卖管理系统-项目架构文档.md` |
| SQL 对象定义 | `docs/sql/sql-objects.sql` |
| 数据库建表脚本 | `takeout-server/takeout-service/src/main/resources/sql/schema.sql` |
